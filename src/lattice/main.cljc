(ns lattice.main
  "Kotodama WASM entrypoint for the Lattice clean-room actor (L4) — Clojure port.
  ...
  facts (`lattice.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "lattice")
(def tier "L4")
...
(def entity-specs
  [{:entity "Employee"   :plural "employees"   :id-prefix "lattice_emp"
    :fields [:firstName :lastName :email :title :department]              :required [:firstName :lastName]
    :coerce {}                       :refs {}}
   {:entity "Candidate"      :plural "candidates"      :id-prefix "lattice_can"
    :fields [:firstName :lastName :email :stage] :required [:firstName :lastName]
    :coerce {}          :refs {}}
   {:entity "JobReq"     :plural "jobreqs"     :id-prefix "lattice_job"
    :fields [:title :department :status :openings]        :required [:title :department]
    :coerce {:openings :int} :refs {}}
   {:entity "PayrollRun" :plural "payrollruns" :id-prefix "lattice_pay"
    :fields [:period :grossTotal :currency :status]          :required [:period :grossTotal]
    :coerce {:grossTotal :float}          :refs {}}
   {:entity "TimeOff"    :plural "timeoffs"    :id-prefix "lattice_tim"
    :fields [:employeeId :type :start :days :status]   :required [:type :start]
    :coerce {:days :float}        :refs {:employeeId "Employee"}}
   {:entity "Review"    :plural "reviews"    :id-prefix "lattice_rev"
    :fields [:employeeId :cycle :rating :reviewerId]  :required [:cycle :rating]
    :coerce {:rating :float}          :refs {:employeeId "Employee"}}])
...
(defn emit-facts
  "EAVT facts for one record: {\"lattice.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "lattice-compat" :tier tier :entities entities} 200])
...
(defn start! [] :lattice-compat/ready)
