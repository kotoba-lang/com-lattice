(ns lattice.main-test
  "Contract + behavioral test for the lattice-compat L4 actor (cljc port).
...
(deftest healthz
  (let [[body status] (m/healthz)]
    (is (= 200 status)) (is (= "lattice-compat" (:actor body))) (is (= (set m/entities) (set (:entities body))))))
